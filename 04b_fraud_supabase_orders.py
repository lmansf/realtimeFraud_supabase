...
from fraud_model import train_and_save, predict_order, MODEL_PATH
...
def stream_to_supabase(interval_seconds=3, max_orders=None):
    ...
    # Train (or retrain) the supervised model on historical + Supabase data
    # before the first order is scored.
    print("Training fraud classifier on historical data…", flush=True)
    train_and_save(eval=False)
    ...
    try:
        while True:
            ...
            # Score the new order with the trained classifier
            prediction = predict_order(order)
            flag = "🚨 FRAUD" if prediction["is_fraud"] else "✅ OK"

            order_count += 1
            print(
                f"\n[{order_count}] {order['order_id']} | {order['restaurant_id']} "
                f"| AED {order['total_amount']} | {flag} "
                f"(p={prediction['fraud_probability']:.3f})",
                flush=True,
            )
